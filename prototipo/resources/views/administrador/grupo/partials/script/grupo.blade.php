<script>
    document.addEventListener('DOMContentLoaded', function() {
        var form = document.getElementById('formularioDatosGrupo');
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('[id^="formularioEdicionGrupo"]').forEach(form => {
            form.addEventListener('submit', function(event) {
                event.preventDefault();
                const successMessage = document.getElementById(
                    `successMessage${form.dataset.grupoId}`);
                successMessage.classList.remove('d-none');
                setTimeout(function() {
                    $(`#editarModal${form.dataset.grupoId}`).modal('hide');
                    successMessage.classList.add('d-none');
                    form.submit();
                }, 2000);
            });
